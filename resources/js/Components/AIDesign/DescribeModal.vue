<template>
  <div class="modal-overlay fixed inset-0 bg-black bg-opacity-50 z-50">
    <div class="modal-content bg-white p-[36px] rounded-lg max-w-xl mx-auto my-[230px]">
      <div class="template-modal w-full flex justify-between items-center mb-8">
        <h1 class="text-xl font-sans1 text-[#2F3144]">Describe your design</h1>
        <div @click="closeDescribeModal" class="cursor-pointer text-[#6245E7]">
      ...
      <div class="mt-8">
        <div class="flex flex-col gap-[20px]">
          <div class="form-control flex flex-col gap-[12px]">
            ...
            <textarea 
              class="w-full h-[93px] p-[10px] rounded-[4px] border-[1px] border-[#E4E7EC] resize-none focus:border-primary-color focus:outline focus:outline-offset-[0px] focus:outline-[2px] focus:outline-[#F4EBFF]"
            >
            </textarea>
          </div>
          <div class="flex justify-end items-center">
            <button 
              class="w-[102px] rounded-[8px] p-[10px] bg-primary-color text-white text-[16px]"
              @click="setOpenTemplateModal"
            >Done</button>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    closeDescribeModal() {
      this.$emit('closeDescribeModal');
    },
    setOpenTemplateModal() {
      this.$emit('setOpenTemplateModal');
    }
  },
};
